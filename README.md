...
> V3 本地演示登录账号：`admin`；初始密码：`NovaERP@2026`。首次启动 V3 后端会自动创建该管理员，数据库仅保存密码哈希。正式部署时务必通过 `ERP_INITIAL_ADMIN_PASSWORD` 环境变量覆盖该初始密码。
...
输入密码时会以 `*` 隐藏。该密码只保存在当前终端的环境变量中，关闭终端后自动失效；不会写入项目文件或 Git。看到 `Started ErpServerApplication` 后，后端已连接 MySQL 并启动成功。
...
首次启动一个空的 V3 数据库时，系统会创建本地演示管理员：`admin` / `NovaERP@2026`。登录页不会显示此凭据；请只在本机演示环境使用，登录后可通过右上角 **改密** 修改。
