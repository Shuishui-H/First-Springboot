...
本仓库是 ERP 项目实训的起点。目前完成了一个可运行的“商品档案 + 采购管理 + 销售管理 + 仓储管理 + 业务报表”垂直切片，用来验证 **Vue 3 前端 → HTTP/JSON → Spring Boot REST API** 的前后端分离开发方式。

样例对应课程 PPT 中“进销存系统”的商品基础档案、采购、销售、仓储、业务报表和系统设置能力。当前版本支持商品增删改查、安全库存预警、供应商管理、采购订单创建/提交/审核/驳回、采购入库、销售客户、销售订单审核、销售出库、销售退货、仓库档案、库存余额、库存流水、手工出入库、到货/履约进度、经营看板、采购分析、库存余额/预警报表、用户管理、角色权限、仓库/供应商/客户主数据维护和 CSV 导出。
...
> V3 课堂演示账号：`admin / admin123`（管理员）、`purchase / 123456`（采购员）、`saler / 123456`（销售员）、`warehouse / 123456`（仓管员）、`finance / 123456`（经营管理者）。首次启动 V3 后端会自动补齐缺失账号，数据库仅保存密码哈希；正式部署时务必通过环境变量覆盖初始密码并关闭演示账号。
...
| 后端 | Spring Boot REST API，提供商品、采购、销售、仓储和业务报表接口；V3 新增登录、调拨、盘点、审计与密码修改接口 |
| 数据 | 商品、供应商、客户、采购订单、销售订单、出入库、退货和系统设置均保存到本机 MySQL；后端重启后自动恢复 |
| 构建 | `mvn test`、`mvn package -DskipTests`、`npm run build` 均已通过 |
| 联调 | 前端代理 `/api` 到后端，商品、采购、销售、仓储和业务报表页面均可调用接口 |
| 接口冒烟 | 已实际验证销售订单闭环和仓储手工出入库的库存联动 |
| MySQL | 已接入 `nova_erp`；启动时 Flyway 自动升级表结构；V3 调拨/盘点、库存余额、库存流水和审计日志同步写入关系表 |
...
V3 已接入登录鉴权、角色权限、调拨/盘点单和操作审计；业务页面通过后端接口访问，MySQL 启动时由 Flyway 管理版本化表结构。
...
```powershell
cd erp-server
$env:DB_PASSWORD = [System.Net.NetworkCredential]::new('', (Read-Host '请输入 MySQL 密码' -AsSecureString)).Password; mvn spring-boot:run
```

输入的是 MySQL 数据库密码，不是 ERP 登录页的 `admin123`；密码会以 `*` 隐藏，只保存在当前终端的环境变量中，关闭终端后自动失效，不会写入项目文件或 Git。看到 `Started ErpServerApplication` 后，后端已连接 MySQL 并启动成功。
...
首次启动一个空的 V3 数据库时，系统会自动创建课堂演示账号。登录页会标注账号密码，点击即可填充；请只在课堂演示环境使用，登录后可通过右上角 **改密** 修改。

数据库里只保存 BCrypt 密码哈希，不保存可还原的登录密码。连续 5 次登录失败会锁定账号 15 分钟。正式部署前必须设置 `ERP_INITIAL_ADMIN_PASSWORD`，覆盖演示初始密码。

### MySQL 备份与恢复

以下命令在 PowerShell 中执行。命令会提示输入数据库密码，输入过程不显示字符；不要把密码写进 `.md`、`.cmd`、源码或 Git。

```powershell
# 备份当前 ERP 数据库到项目外的备份文件
mysqldump -u root -p --single-transaction --routines --triggers nova_erp > "$env:USERPROFILE\Desktop\nova_erp_backup.sql"

# 先创建一个用于验证恢复的空数据库，再导入备份
mysql -u root -p -e "CREATE DATABASE nova_erp_restore CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci"
Get-Content "$env:USERPROFILE\Desktop\nova_erp_backup.sql" | mysql -u root -p nova_erp_restore
```

恢复验证库后，可在启动后端前把 `$env:DB_NAME` 临时设置为 `nova_erp_restore`，确认数据无误后关闭终端即可恢复默认数据库名。不要将恢复命令直接执行到正在使用的 `nova_erp`，除非已经确认备份可用并明确要覆盖当前数据。
...
| 问题 | 影响 | 后续处理 |
| --- | --- | --- |
| 采购/销售领域服务仍保留兼容快照边界 | 关系表与旧业务历史的迁移仍需逐模块对账 | V3 稳定验收后移除快照兼容层 |
| 列表默认未统一分页 | 数据量较大时响应会变慢 | V4 增加分页查询与分页组件 |
| 自动化测试仍需扩充 | 回归依赖部分人工操作 | 增加 Service 与 Controller 集成测试 |
